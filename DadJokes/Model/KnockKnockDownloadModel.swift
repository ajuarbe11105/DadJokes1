...
class Knock: ObservableObject {
    
@Published var knock: [KnockKnock] = []
    ...
    func thereJokes() {
        ...
        guard let url = URL(string: "https://us-central1-dadsofunny.cloudfunctions.net/DadJokes/random/type/knock-knock") else { return }
                            //We got this url from /post1 from jsonplaceholder
        
        downloadData(fromURL: url) { (returnedData) in
            if let data = returnedData {
                guard let newPosts = try? JSONDecoder().decode([KnockKnock].self, from: data) else { return }
